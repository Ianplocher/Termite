const steps = [
  {
    number: "1",
    title: "Book Online",
    description: "Pick a date and time that works for you. It takes less than 60 seconds.",
    icon: (
      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
    ),
  },
  {
    number: "2",
    title: "We Come To You",
    description: "A licensed termite inspector visits your Riverside property at your scheduled time.",
    icon: (
      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
      </svg>
    ),
  },
  {
    number: "3",
    title: "Get Your Report",
    description: "Receive a full written inspection report, completely free of charge.",
    icon: (
      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
    ),
  },
];

export default function HowItWorks() {
  return (
    <section className="py-16 md:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-12">
          {/* Left side - image */}
          <div className="relative rounded-2xl overflow-hidden shadow-xl">
            <div
              className="w-full h-72 md:h-96 bg-cover bg-center"
              style={{ backgroundImage: "url('/inspector-home.jpg')" }}
            />
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-navy/80 to-transparent p-6">
              <p className="text-white font-semibold text-lg">Don&apos;t let termite damage go undetected</p>
            </div>
          </div>

          {/* Right side - text */}
          <div>
            <h2 className="text-3xl md:text-4xl font-bold text-navy mb-4">
              How Your Free Termite Inspection Works
            </h2>
            <p className="text-lg text-gray-600">
              Getting a professional termite inspection in Riverside has never been easier. Three simple steps to protect your home.
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((step) => (
            <div
              key={step.number}
              className="relative text-center p-8 rounded-xl bg-gray-50 border border-gray-100 hover:shadow-lg transition-shadow"
            >
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-orange text-white mb-6">
                {step.icon}
              </div>
              <div className="absolute top-4 right-4 text-5xl font-bold text-gray-100">
                {step.number}
              </div>
              <h3 className="text-xl font-bold text-navy mb-3">{step.title}</h3>
              <p className="text-gray-600">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
